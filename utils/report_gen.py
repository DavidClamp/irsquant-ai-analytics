...
class DailyRiskReportGenerator:
    ...
    def generate_eod_snapshot(self):
        ...
        with open(report_path, "w", encoding="utf-8") as md:
            ...
            md.write(
                f"| Position Currency | Structural Fly Dislocation | Hedge Ratio (S/L) | R-Squared | Z-Score | Trading Signal Trigger |\n")
            md.write(
                f"|:------------------|:----------------------------|:------------------|:----------|:--------|:-----------------------|\n")
            ...
            for ccy in GLOBAL_UNIVERSE:
                try:
                    ...
                    if leaderboard and len(leaderboard) > 0:
                        ...
                        md.write(
                            f"| **{ccy}** | {top_trade['Structure']} | {top_trade['Hedge Ratio']} | {top_trade['R-Squared']:.4f} | {z_val:.2f} | `{signal_str}`{alert_flag} |\n")
                    else:
            ...
            md.write(
                f"* This report object contains completely proprietary data. Local file locked out automatically on hard drive cluster.")
