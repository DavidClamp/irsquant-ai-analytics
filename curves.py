...
class BootstrappedDiscountCurve:
    ...
    def __init__(self, target_date, spot_rates_dict):
        ...
        # Map out standardized liquid G4 maturity nodes (in year fractions)
        self.tenor_map = {
            0.25: '3M', 1.0: '1Y', 2.0: '2Y', 3.0: '3Y', 
            4.0: '4Y', 5.0: '5Y', 7.0: '7Y', 10.0: '10Y'
        }
        self.maturities = sorted(list(self.tenor_map.keys()))
        self.spot_rates = spot_rates_dict  # Dictionary mapping e.g., '1Y': 0.045
        
        # Core Output Matrix Maps
    ...
    def construct_piecewise_discount_curve(self):
        ...
        """
        # Day 0 anchor condition: Present value of 1 compounding cash unit today is exactly 1.0
        self.discount_factors[0.0] = 1.0
        
        # 1. Short-Term Money Market Node (3M Cash / Money Market Proxy)
        r_3m = self.spot_rates.get('3M', 0.0)
        self.discount_factors[0.25] = 1.0 / (1 + r_3m * 0.25)
        
        # 2. Sequential Annual Swap Bootstrapping Loops
        accumulated_annuity = 0.0
        
        for t in self.maturities:
            if t == 0.25:
                continue  # Short end handled above
                
            r_t = self.spot_rates.get(self.tenor_map[t], 0.0)
            
            if t <= 5.0:
                # Annual payment frequency node solving track
                # Solve for current node T using previous calculated discount anchors
                ...
                # Long-End Structural Interpolation Nodes (7Y and 10Y gaps)
                # Mimics basic institutional curve smoothing to resolve missing payment years
                prev_t = 5.0 if t == 7.0 else 7.0
                gap = int(t - prev_t)
                
                # Approximate intervening unquoted discount structures using log-linear decay lines
    ...
    def get_discount_factor(self, T):
        ...
        Guarantees that your analytics models can price unmapped, fractional forward-starting contracts.
        ...
        if T > 10.0:
            # Simple constant forward extension bounding rule for long-end tails
            return self.discount_factors[10.0] * np.exp(-self.spot_rates.get('10Y', 0.0) * (T - 10.0))
            
        # Locate surrounding interpolation anchor coordinates
        ...
        # Execute continuous Log-Linear interpolation
    ...
    def get_annuity_factor(self, start_n, tenor_m, payment_freq=1.0):
        ...
        This multiplier is mandatory for pricing Swaptions/Caps via Black '76 models.
