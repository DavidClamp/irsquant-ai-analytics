            ...
            dbc.Row(
                ...
                children=[
                    dbc.Col(md=6, children=[
                        ...
                        html.P("Parametric SABR Surface Mapper Powered by Native QuantLib Optimization Loops",
                    ...
                    dbc.Col(md=3, children=[
                        ...
                        dcc.Dropdown(
                            ...
                            options=[{"label": f"{ccy} Options Grid", "value": ccy}
                    ...
                    dbc.Col(
                        ...
                        children=[
                            dbc.Card(
                                ...
                                children=[
                                    dcc.Graph(id="swap-sabr-3d-surface-canvas",
                                              style={'height': '600px'}, config={'displayModeBar': False})
                                ]
                            )
                        ]
                    )
...
def register_swaption_callbacks(app):
    ...
    def regenerate_3d_volatility_surface(currency, target_date):
        ...
        try:
            with open("data/calibrated_sabr_surfaces.json", "r") as f:
                cache = json.load(f)
            params = cache.get(currency.upper(), {}).get(
                target_date, {"alpha": 0.2550, "beta": 0.5000, "rho": -0.2500, "nu": 0.1500})
        except Exception:
        ...
        metrics_cards = [
            dbc.Col(md=3, children=[dbc.Card(style={'backgroundColor': '#11141a', 'border': '1px solid #22293a'}, className="p-2", children=[html.Small(
                "SABR Alpha (ATM Grounding Scale)", className="text-muted small"), html.H5(f"{alpha:.4f}", className="text-success fw-bold m-0")])]),
            dbc.Col(md=3, children=[dbc.Card(style={'backgroundColor': '#11141a', 'border': '1px solid #22293a'}, className="p-2", children=[
                    html.Small("SABR Beta (CEV Exponent Locked)", className="text-muted small"), html.H5(f"{beta:.4f}", className="text-white fw-bold m-0")])]),
            dbc.Col(md=3, children=[dbc.Card(style={'backgroundColor': '#11141a', 'border': '1px solid #22293a'}, className="p-2", children=[
                    html.Small("SABR Rho (Smile Skew Direction)", className="text-muted small"), html.H5(f"{rho:.4f}", className="text-warning fw-bold m-0")])]),
            dbc.Col(md=3, children=[dbc.Card(style={'backgroundColor': '#11141a', 'border': '1px solid #22293a'}, className="p-2", children=[
                    html.Small("SABR Nu (Vol-Of-Vol Volatility)", className="text-muted small"), html.H5(f"{nu:.4f}", className="text-info fw-bold m-0")])])
        ]
