            ...
            dbc.Row(
                ...
                children=[
                    dbc.Col(md=4, children=[
                        ...
                        html.P("Intra-Curve Structural Deviation Analysis | Cross-Asset Leaderboard",
                    ...
                    dbc.Col(md=2, children=[
                        dbc.Button("🟢 Trigger Matrix Sweep", id="scanner-sweep-btn",
                                   color="success", className="w-100 fw-bold pt-2 pb-2 mt-4")
                    ])
                ]
            ),
                    ...
                    dbc.Col(
                        ...
                        children=[
                            dbc.Card(
                                ...
                                children=[
                                    html.Div("🤖 Natural Language Macro AI Co-Pilot Terminal", style={
                                             'color': '#00d2ff', 'fontWeight': 'bold', 'fontFamily': 'monospace', 'fontSize': '12px', 'marginBottom': '8px'}),
                                    dbc.InputGroup(
                                        children=[
                                            dbc.Input(id="scanner-ai-input", placeholder="Type macro prompt (e.g., 'Scan all books' or 'Show cheap USD flies')...",
                                                      className="bg-dark text-white border-secondary"),
                                            dbc.Button("🔮 Parse Intent & Run AI Scan", id="scanner-ai-btn",
                                                       color="purple", className="fw-bold px-4")
                                        ]
                                    )
                                ]
                            )
                    ...
                    dbc.Col(
                        ...
                        children=[
                            dbc.Card(
                                ...
                                children=[
                                    html.H5("Intra-Curve Statistical Arbitrage Dislocation Leaderboard",
                                            className="text-white monospace mb-3", style={'fontSize': '14px'}),
                                    html.Div(id="scanner-matrix-table-slot", children=[
                                        html.P("Select currency parameters above and click 'Trigger Matrix Sweep' to map yield curve anomalies.",
                                               className="text-muted small font-monospace m-0")
                                    ])
                                ]
                            )
                        ]
    ...
    def execute_single_currency_curve_scan(sweep_clicks, ai_clicks, selected_view, ai_prompt):
        ...
        try:
            ...
            target_flies = [
                ...
                {"name": "2Y / 5Y / 10Y Benchmark Fly", "s": "2Y", "b": "5Y",
                    "l": "10Y", "s_f": "1Y", "b_f": "4Y", "l_f": "9Y"},
                {"name": "3Y / 5Y / 7Y Mid Curve Fly", "s": "3Y", "b": "5Y",
                    "l": "7Y", "s_f": "2Y", "b_f": "4Y", "l_f": "6Y"},
                {"name": "5Y / 7Y / 10Y Core Fly", "s": "5Y", "b": "7Y", "l": "10Y", "s_f": "4Y", "b_f": "6Y", "l_f": "9Y"},
                {"name": "5Y / 10Y / 30Y Long Wing Fly", "s": "5Y", "b": "10Y",
                    "l": "30Y", "s_f": "4Y", "b_f": "9Y", "l_f": "29Y"}
            ]
                        ...
                        for t_num in range(min(numeric_tenors), max(numeric_tenors) + 1):
                            ...
                            if t_lbl not in df_hist_pivot.columns:
                                ...
                                if lower_n and upper_n:
                                    ...
                                    df_hist_pivot[t_lbl] = df_hist_pivot[f"{p1}Y"] + (
                                        df_hist_pivot[f"{p2}Y"] - df_hist_pivot[f"{p1}Y"]) * ((t_num - p1) / (p2 - p1))
                for fly in target_flies:
                    ...
                    if all(v is not None for v in [r_s, r_b, r_l]):
                        ...
                        if not df_hist_pivot.empty and {fly["s"], fly["b"], fly["l"]}.issubset(df_hist_pivot.columns):
                            hist_fly_series = ((2.0 * df_hist_pivot[fly["b"]]) -
                                               df_hist_pivot[fly["s"]] - df_hist_pivot[fly["l"]]) * 100.0
                            ...
                            if {fly["s_f"], fly["b_f"], fly["l_f"]}.issubset(df_hist_pivot.columns):
                                shorter_fly_series = (
                                    (2.0 * df_hist_pivot[fly["b_f"]]) - df_hist_pivot[fly["s_f"]] - df_hist_pivot[fly["l_f"]]) * 100.0
            ...
            return dbc.Table(
                ...
                children=[
                    html.Thead(html.Tr([
                        html.Th("Currency"), html.Th("Curve Asset Structure"), html.Th(
                            "Short Leg"), html.Th("Belly Center"), html.Th("Long Leg"),
                    ...
                    html.Tbody([
                        html.Tr([
                            ...
                            html.Td(row["Short"], className="text-white-50"), html.Td(row["Belly"],
                                                                                      className="text-white"), html.Td(row["Long"], className="text-white-50"),
                            html.Td(f"{row['Spread']:+.2f} bp", className="text-info fw-bold"),
                            html.Td(f"{row['Z']:+.2f} σ", style={'color': '#af52de', 'fontWeight': 'bold'}),
                            html.Td(f"{row['Pct']:.1f}%", className="fw-bold text-warning" if (row['Pct']
                                    >= 90.0 or row['Pct'] <= 10.0) else "text-white"),
                            # 🟢 Render your clean 1Y Forward Roll metrics with crisp color highlights
                            html.Td(f"{float(row['FwdRoll']):+.1f} bp",
                                    className="text-success fw-bold" if float(row['FwdRoll']) > 0 else "text-danger"),
