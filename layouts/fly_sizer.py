            ...
            dbc.Row(
                ...
                children=[
                    dbc.Col(md=6, children=[
                        ...
                        html.P("DV01 Risk-Targeted Fly Balancers, Notional Solvers & Order Booking",
                    ...
                    dbc.Col(md=3, children=[
                        ...
                        dcc.Dropdown(
                            ...
                            options=[{"label": f"{ccy} Trading Book", "value": ccy}
                    ...
                    dbc.Col(md=3, children=[
                        ...
                        dcc.Dropdown(
                            ...
                            options=[{"label": b, "value": b}
                                     for b in ["Macro-RV-Fly", "STIR-Hedging", "Exotics-Match"]],
                            value="Macro-RV-Fly",
                            clearable=False,
                            className="text-dark fw-bold"
                        )
                    ...
                    dbc.Col(
                        ...
                        children=[
                            dbc.Card(
                                ...
                                children=[
                                    html.H5("3-Leg Risk-Targeted Butterfly Ticket", className="text-pink monospace mb-4",
                                            style={'fontSize': '14px', 'color': '#ff1a75'}),

                                    dbc.Row(className="g-2 mb-3", children=[
                                        dbc.Col(md=4, children=[
                                            html.Label("Short Wing", className="text-muted small mb-1"),
                                            dbc.Select(id="fly-short-tenor", options=fly_options, value="1",
                                                       className="bg-dark text-white border-secondary")
                                        ]),
                                        dbc.Col(md=4, children=[
                                            html.Label("Belly Anchor", className="text-muted small mb-1"),
                                            dbc.Select(id="fly-mid-tenor", options=fly_options, value="2",
                                                       className="bg-dark text-white border-secondary")
                                        ]),
                                        dbc.Col(md=4, children=[
                                            html.Label("Long Wing", className="text-muted small mb-1"),
                                            dbc.Select(id="fly-long-tenor", options=fly_options, value="5",
                                                       className="bg-dark text-white border-secondary")
                                        ])
                                    ]),

                                    html.Div(className="mb-3", children=[
                                        html.Label("Target Risk Sensitivity (DV01 $/bp)",
                                                   className="text-warning small fw-bold mb-1"),
                                        dbc.Input(id="fly-target-dv01", type="number", value=10000,
                                                  step=500, className="bg-dark text-white border-warning")
                                    ]),

                                    dbc.Row(className="g-2 mb-4", children=[
                                        dbc.Col(md=6, children=[
                                            html.Label("Short Risk Weight (Beta)", className="text-muted small mb-1"),
                                            dbc.Input(id="fly-short-beta", type="number", value=0.50,
                                                      step=0.05, className="bg-dark text-white border-secondary")
                                        ]),
                                        dbc.Col(md=6, children=[
                                            html.Label("Long Risk Weight (Beta)", className="text-muted small mb-1"),
                                            dbc.Input(id="fly-long-beta", type="number", value=0.50,
                                                      step=0.05, className="bg-dark text-white border-secondary")
                                        ])
                                    ]),

                                    dbc.Button("Calibrate Fly Sizing", id="fly-calc-btn", color="danger",
                                               className="w-100 fw-bold mb-2", style={'backgroundColor': '#ff1a75', 'borderColor': '#ff1a75'}),
                                ]
                            ),

                            # ORDER DISPATCH SHIELD TRIGGER
                            dbc.Button("Submit Butterfly Order Block", id="fly-book-btn", color="outline-danger",
                                       className="w-100 fw-bold p-2", style={'borderColor': '#ff1a75', 'color': '#ff1a75'}),
                    ...
                    dbc.Col(
                        ...
                        children=[
                            dbc.Card(
                                ...
                                children=[
                                    html.H5("Execution Desk Risk Summary Matrix",
                                            className="text-white monospace mb-4", style={'fontSize': '14px'}),
    ...
    def compute_butterfly_notional_allocations(n_clicks, currency, short_t, mid_t, long_t, target_dv01, beta_s, beta_l):
        ...
        return html.Div(
            ...
            children=[
                ...
                html.Div(children=[
                    html.H6(f"Live Market Swap Coupons & Net Spread ({currency})",
                            className="text-warning monospace mb-2", style={'fontSize': '13px'}),
                    dbc.Row(className="g-2 text-center", children=[
                        dbc.Col(md=3, children=[html.Div(className="p-2 bg-dark rounded border border-secondary", children=[html.Small(
                            f"{short_t}Y Coupon", className="text-muted small"), html.H5(f"{r_short:.4f}%", className="text-white fw-bold m-0")])]),
                        dbc.Col(md=3, children=[html.Div(className="p-2 bg-dark rounded border border-secondary", children=[html.Small(
                            f"{mid_t}Y Coupon", className="text-muted small"), html.H5(f"{r_mid:.4f}%", className="text-white fw-bold m-0")])]),
                        dbc.Col(md=3, children=[html.Div(className="p-2 bg-dark rounded border border-secondary", children=[html.Small(
                            f"{long_t}Y Coupon", className="text-muted small"), html.H5(f"{r_long:.4f}%", className="text-white fw-bold m-0")])]),
                        dbc.Col(md=3, children=[html.Div(className="p-2 bg-dark rounded border border-info", style={'backgroundColor': '#11141a'}, children=[
                                html.Small("Net Fly Spread", className="text-info small"), html.H5(f"{net_fly_spread_bps:+.2f} bps", className="text-info fw-bold m-0")])])
                    ])
                ]),

                # RISK-BALANCED NOTIONAL TICKETS
                html.Div(children=[
                    html.H6("DV01 Duration-Weighted Notional Sizing Output",
                            className="monospace mb-2", style={'color': '#ff1a75', 'fontSize': '13px'}),
                    dbc.Row(className="g-2 text-center", children=[
                        dbc.Col(md=4, children=[html.Div(className="p-2 bg-dark rounded border border-secondary", children=[html.Small(
                            f"Short Notional ({short_t}Y)", className="text-muted small"), html.H5(f"${short_allocated_m:,.2f}M", className="text-danger fw-bold m-0")])]),
                        dbc.Col(md=4, children=[html.Div(className="p-2 bg-dark rounded border border-success", children=[html.Small(
                            f"Belly Notional ({mid_t}Y)", className="text-muted small"), html.H5(f"-${mid_allocated_m:,.2f}M", className="text-success fw-bold m-0")])]),
                        dbc.Col(md=4, children=[html.Div(className="p-2 bg-dark rounded border border-secondary", children=[html.Small(
                            f"Long Notional ({long_t}Y)", className="text-muted small"), html.H5(f"${long_allocated_m:,.2f}M", className="text-danger fw-bold m-0")])])
                    ])
                ]),
