            ...
            dbc.Row(
                ...
                children=[
                    dbc.Col(md=6, children=[
                        ...
                        html.P("Multi-Leg Sizing Engines, Present Value Basis Risks & Structural Spread Adjustments",
                    ...
                    dbc.Col(
                        ...
                        children=[
                            dbc.Card(
                                ...
                                children=[
                                    html.H5("2-Leg Basis Sizing Ticket",
                                            className="text-white monospace mb-4", style={'fontSize': '14px'}),

                                    html.Div(className="mb-3", children=[
                                        html.Label("Leg 1: Target Notional (Millions)",
                                                   className="text-muted small mb-1"),
                                        dbc.Input(id="exec-notional-input", type="number", value=100.0,
                                                  step=5.0, className="bg-dark text-white border-secondary")
                                    ]),

                                    dbc.Row(className="g-2 mb-4", children=[
                                        dbc.Col(md=6, children=[
                                            html.Label("Leg 1 Tenor", className="text-muted small mb-1"),
                                            dbc.Select(id="exec-tenor-leg1", options=tenor_options, value="2",
                                                       className="bg-dark text-white border-secondary")
                                        ]),
                                        dbc.Col(md=6, children=[
                                            html.Label("Leg 2 Balance Tenor", className="text-muted small mb-1"),
                                            dbc.Select(id="exec-tenor-leg2", options=tenor_options,
                                                       value="10", className="bg-dark text-white border-secondary")
                                        ])
                                    ]),

                                    dbc.Button("Calculate Sizing & Risk", id="exec-calc-btn",
                                               color="success", className="w-100 fw-bold mb-2"),
                                ]
                            ),
                            dbc.Button("Submit Basis Swap Order Block", id="exec-book-btn",
                                       color="outline-success", className="w-100 fw-bold p-2"),
                            ...
                            dbc.Card(
                                ...
                                children=[
                                    ...
                                    html.Div(id="exec-risk-results-container", children=[
                                        html.P("Enter trade sizing components and click 'Calculate Sizing & Risk' to model PVBP curve exposures.",
                                               className="text-muted small monospace m-0")
                                    ])
                                ]
                            )
                        ]
    ...
    def run_execution_sizing_model(n_clicks, currency, notional, tenor1, tenor2):
        ...
        return html.Div(
            children=[
                ...
                html.H6(f"Live Market Swap Coupons & Curve Spread ({currency})",
                        className="text-warning monospace mb-3", style={'fontSize': '13px'}),
                dbc.Row(className="g-2 text-center mb-4", children=[
                    dbc.Col(md=4, children=[html.Div(className="p-2 bg-dark rounded border border-secondary", children=[html.Small(
                        f"{tenor1}Y Coupon (Leg 1)", className="text-muted small"), html.H5(f"{rate_leg1:.4f}%", className="text-white fw-bold m-0")])]),
                    dbc.Col(md=4, children=[html.Div(className="p-2 bg-dark rounded border border-secondary", children=[html.Small(
                        f"{tenor2}Y Coupon (Leg 2)", className="text-muted small"), html.H5(f"{rate_leg2:.4f}%", className="text-white fw-bold m-0")])]),
                    dbc.Col(md=4, children=[html.Div(className="p-2 bg-dark rounded border border-info", style={'backgroundColor': '#11141a'}, children=[
                            html.Small("Basis Curve Spread", className="text-info small"), html.H5(f"{curve_spread_bps:+.2f} bps", className="text-info fw-bold m-0")])])
                ]),


                # RE-CALIBRATED DV01 RISK MATCHING CARD PANELS
                html.H6("DV01 Risk-Neutral Allocation Summary",
                        className="text-success monospace mb-3", style={'fontSize': '13px'}),
                dbc.Row(className="mb-3 g-2", children=[
                    dbc.Col(md=6, children=[html.Div(className="p-3 bg-dark border rounded text-center", children=[html.Small(
                        f"Leg 1 ({tenor1}Y) Total DV01", className="text-muted small d-block mb-1"), html.H4(f"${leg1_dv01:,.2f}", className="text-white fw-bold m-0")])]),
                    dbc.Col(md=6, children=[html.Div(className="p-3 bg-dark border rounded text-center", children=[html.Small(
                        f"Leg 2 ({tenor2}Y) Risk Target", className="text-muted small d-block mb-1"), html.H4(f"${leg2_dv01:,.2f}", className="text-white fw-bold m-0")])])
                ]),
                    ...
                    children=[
                        dbc.Row(className="align-items-center g-3", children=[
                            ...
                            dbc.Col(md=8, className="ps-3", children=[
                                html.Small("Sizing Execution Recommendation",
                                           className="text-muted d-block small mb-1"),
                                html.P([
                                    "Trade exactly ",
